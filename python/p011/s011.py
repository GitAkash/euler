...
def parse(grid):
    ...
    transposed_grid = []
    while i < 20:
        l = ([int(i) for i in grid.split()])
        new_grid.append(l[20*i:20+20*i])
        transposed_grid.append(l[i::20])
        i += 1
    return (transposed_grid, new_grid)
...
def greatestproduct(n):
    ...
    for i in range(20):
        for j in range(20):
            # Horizontal Check
            horizon = grid[i][j:j+4]
            multH = multiplylist(horizon) 
            # Vertical Check
